#!/usr/bin/env bash
# A self-contained tour of praetor's trust model — no Claude session needed.
...
[ -x "$BIN/praetor-mcp" ] || { echo "build first: cargo build --release"; exit 1; }

say "1. Each agent has an Ed25519 identity — the public key IS the id."
run "praetor-keygen --out {alice,bob,eve}.key"
ALICE=$("$BIN/praetor-keygen" --out "$D/alice.key" | awk '/^public key/{print $4}')
BOB=$(  "$BIN/praetor-keygen" --out "$D/bob.key"   | awk '/^public key/{print $4}')
EVE=$(  "$BIN/praetor-keygen" --out "$D/eve.key"   | awk '/^public key/{print $4}')
...
run "praetor-bus --addr 127.0.0.1:9440"
"$BIN/praetor-bus" --addr 127.0.0.1:9440 >"$D/bus.log" 2>&1 & BUS=$!
sleep 0.5
...
bob_listen() {
  ( printf '%s\n' \
    ...
    | PRAETOR_KEY="$D/bob.key" PRAETOR_PEERS="$D/bob-peers.json" PRAETOR_URL=http://127.0.0.1:9440 \
      "$BIN/praetor-mcp" 2>"$D/bob.err"
}
send_as() { # $1=keyfile $2=text
  ( printf '%s\n' \
    ...
    | PRAETOR_KEY="$1" PRAETOR_PEERS="$D/to-bob.json" PRAETOR_URL=http://127.0.0.1:9440 \
      "$BIN/praetor-mcp" >/dev/null 2>&1 || true
}
