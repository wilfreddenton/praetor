...
The workdir must contain, before running:
    bin/               interlink-mcp, interlink-keygen, interlink-bus
    alice.key bob.key  from interlink-keygen
    bob-peers.json     allowlisting alice (e.g. {"alice":{"key":"…","may":"*"}})
    alice-peers.json   allowlisting bob
    bobdir/bob.mcp.json  MCP config defining the `interlink` server for bob
and a bus must be running on INTERLINK_URL (default http://127.0.0.1:9440).
"""
import os, pty, select, time, re, subprocess, sys

T = os.environ["INTERLINK_TEST_DIR"]
BIN = os.path.join(T, "bin")
URL = os.environ.get("INTERLINK_URL", "http://127.0.0.1:9440")
cap = []
...
def peer_sends(text):
    ...
    env = dict(os.environ, INTERLINK_KEY=f"{T}/alice.key", INTERLINK_PEERS=f"{T}/alice-peers.json", INTERLINK_URL=URL)
    subprocess.run([f"{BIN}/interlink-mcp"], input=reqs.encode(), env=env,
...
def main():
    ...
    if pid == 0:
        ...
        os.execvp("claude", ["claude", "--mcp-config", "bob.mcp.json",
                             "--dangerously-load-development-channels", "server:interlink"])
        return
    ...
    pump(fd, 7)
    os.write(fd, b"You are the agent bob. If a <channel source=\"interlink\"> message arrives, "
